...
resume_path = os.path.join(ckpt_dir, f'{model_name}_best.pth')
if os.path.exists(resume_path):
    map_location = {'cuda:%d' % 0: 'cuda:%d' % local_rank}
    resume_ckpt = torch.load(resume_path, map_location=map_location)
...
for i in range(start_epoch, EPOCHS+1):
    ...
    train_bar = tqdm(
        ...
        desc=f'Epoch {i}/{EPOCHS} [Train]',
        disable=not is_master
    )

    for index, (data, labels) in train_bar:
        index += 1
        ...
        train_bar.set_postfix({
            'loss': f'{running_loss / index:.6f}',
            'acc': f'{100 * cum_acc / index:.4f}%'
        })
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        val_bar = tqdm(
            ...
            desc=f'Epoch {i}/{EPOCHS} [Val]',
            disable=not is_master
        )

        with torch.no_grad():
            for index, (data_v, labels_v) in val_bar:
                index += 1
                ...
                val_bar.set_postfix({'val_loss': f'{running_loss / index:.6f}'})
